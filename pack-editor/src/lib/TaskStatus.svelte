...
{#if taskFeedback.active}
	...
	>
		...
		<span class="message"
			>{task.message}{#if task.current !== null && task.total !== null}
				{task.current}/{task.total}{/if}</span
		>
		{#if taskFeedback.queuedCount}<span
				class="queued"
				title={`${taskFeedback.queuedCount} more status message${taskFeedback.queuedCount === 1 ? '' : 's'}`}
				>+{taskFeedback.queuedCount}</span
			>{/if}
		{#if task.tone === 'error' || task.tone === 'warning'}<button
				onclick={() => taskFeedback.dismiss(task.id)}
				aria-label="Dismiss status"><Icon src={XMark} mini size="13px" /></button
			>{/if}
	</div>
{/if}
