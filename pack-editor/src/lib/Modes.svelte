...
>
	...
	{:else if modes.length === 0}
		<EmptyState
			...
			description="Add a built .lwmode file to distribute its Lua scripts and option specification with this pack."
			...
			{#each modes as mode (mode.id)}
				<article>
					...
					<div class="mode-copy">
						<h3>{mode.name}</h3>
						<p>
							{[mode.author, mode.version ? `Version ${mode.version}` : null]
								.filter(Boolean)
					...
					{#if recommendedValue === `pack:${mode.id}`}<span class="recommended">Recommended</span
						>{/if}
					<Button
						size="compact"
						variant="quiet"
						...
						title="Remove mode"
						onclick={() => (removing = mode)}><Icon src={Trash} mini size="15px" /></Button
					>
				</article>
			{/each}
		</div>
...
{#if removing}
	<Dialog
		...
		description="The mode’s Lua scripts and option specification will be removed from this pack. You can undo this change from the editor history."
		buttons={[
			{ label: 'Cancel', onclick: () => (removing = null) },
			{ label: 'Remove mode', destructive: true, onclick: confirmRemove }
		]}
