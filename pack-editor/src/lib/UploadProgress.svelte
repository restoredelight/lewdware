  ...
  onMount(() => {
    ...
    if (savedValue !== null) {
      ...
      if (Number.isFinite(saved)) {
        ...
        dockedRight = localStorage.getItem("pack-editor:import-window-docked-right") === "true"
...
<div bind:this={windowElement} class="import-window" class:minimized class:dragging style={left === null ? undefined : `left:${left}px;right:auto`} role="status" aria-live="polite">
  <header class="titlebar" role="group" aria-label="Import window controls; drag to move" onpointerdown={startDrag} onpointermove={moveDrag} onpointerup={stopDrag} onpointercancel={stopDrag}>
...
<style>
  ...
  .titlebar { display: flex; align-items: center; gap: 8px; height: 30px; padding: 0 10px; border-bottom: 1px solid var(--ui-border); background: var(--ui-surface-raised); border-radius: var(--ui-radius-md) var(--ui-radius-md) 0 0; cursor: grab; touch-action: none; }
