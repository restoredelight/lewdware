<script lang="ts">
  ...
  $effect(() => {
    active;
    tablist?.querySelector<HTMLElement>('[role="tab"][aria-selected="true"]')?.scrollIntoView({ block: "nearest", inline: "nearest" });
  });
  ...
  function startDrag(event: PointerEvent, index: number) {
    ...
    initialScroll = scrollContainer ? (horizontalDrag ? scrollContainer.scrollLeft : scrollContainer.scrollTop) : 0;
  ...
  function retarget() {
    ...
    const delta = scrollContainer ? (horizontalDrag ? scrollContainer.scrollLeft : scrollContainer.scrollTop) - initialScroll : 0;
...
<div bind:this={tablist} class="stage-tabs" class:reordering={dragging !== null} class:settling role="tablist" aria-label="Experience stages" tabindex="-1">
  {#each stages as stage, index (stage.id)}
    <div role="presentation" style={`--drag-offset:${previewOffset(index)}px`} class:active={active === stage.id} class:dragging={dragging === index} class="stage-item">
      ...
      <button class="stage-tab" role="tab" aria-selected={active === stage.id} tabindex={active === stage.id ? 0 : -1} onclick={() => onselect(stage.id)}>
    ...
    {#if transitionAfter(index)}
      ...
      <div class="transition-item" class:active={active === transition.id} class:hidden={dragging !== null} role="presentation">
        <button class="transition-tab" role="tab" aria-selected={active === transition.id} tabindex={active === transition.id ? 0 : -1} title={transition.duration_seconds === 0 ? "Immediate transition" : `Transition over ${transition.duration_seconds} seconds`} onclick={() => onselect(transition.id)}>
          {transition.duration_seconds === 0 ? "immediate" : formatSeconds(transition.duration_seconds)}
        </button>
      </div>
    {/if}
  {/each}
...
<style>
  ...
  .stage-tabs{position:relative;display:flex;min-width:0;flex-direction:column;gap:2px}.stage-tabs::before{position:absolute;z-index:0;top:18px;bottom:18px;left:50%;width:1px;background:var(--ui-border-strong);content:""}.stage-item{position:relative;z-index:1;display:flex;min-width:0;min-height:44px;align-items:center;border:1px solid var(--ui-border);border-radius:var(--ui-radius-sm);background:var(--ui-bg);color:var(--ui-text);transition:transform 120ms ease}.stage-item:has(.menu-trigger[aria-expanded="true"]){z-index:2}.stage-item:hover{background:var(--ui-surface-raised)}.stage-item.active{background:var(--ui-surface-raised);border-color:var(--ui-border-strong);color:var(--ui-text);box-shadow:inset 2px 0 0 var(--ui-accent-hover)}.stage-item.active .stage-tab{font-weight:600}.stage-tabs.reordering .stage-item{transform:translateY(var(--drag-offset));pointer-events:none}.stage-tabs.reordering .stage-item:not(.dragging):not(.active):hover,.stage-tabs.settling .stage-item:not(.active):hover{background:var(--ui-bg)}.stage-tabs.settling .stage-item{transition:none}.stage-item.dragging{z-index:2;background:var(--ui-surface-raised);opacity:.7}.stage-item.active.dragging{background:var(--ui-surface-raised)}.drag{display:grid;width:26px;height:30px;margin-left:2px;flex:none;padding:0;touch-action:none;place-items:center;border:0;background:transparent;color:currentColor;opacity:.5;cursor:grab}.drag:active{cursor:grabbing}.drag :global(svg){width:15px;height:15px}.stage-tab{display:flex;min-width:0;flex:1;padding:6px 2px;flex-direction:column;gap:2px;border:0;background:transparent;color:inherit;font:inherit;font-size:13px;text-align:left;cursor:pointer}.stage-label{width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.stage-meta{width:100%;overflow:hidden;color:var(--ui-muted);font-family:var(--ui-font-mono);font-size:10px;text-overflow:ellipsis;white-space:nowrap}.stage-tab:focus-visible,.menu-trigger:focus-visible,.drag:focus-visible,.transition-tab:focus-visible{outline:2px solid var(--ui-focus);outline-offset:-2px}.menu-trigger{display:grid;width:30px;height:30px;flex:none;padding:0;place-items:center;border:0;border-radius:4px;background:transparent;color:inherit;cursor:pointer}.menu-trigger:hover{background:rgb(255 255 255/.1)}.transition-item{position:relative;z-index:1;display:flex;min-height:40px;align-items:center;justify-content:center;color:var(--ui-muted);transition:opacity 80ms}.transition-item::before{content:"";position:absolute;top:-3px;bottom:-3px;left:50%;width:1px;margin-left:-.5px;background:transparent}.transition-item.active::before{background:var(--ui-accent)}.transition-item.hidden{opacity:0;pointer-events:none}.transition-tab{position:relative;display:block;min-width:0;padding:3px 8px;border:0;border-radius:var(--ui-radius-sm);background:var(--ui-surface);color:inherit;font-family:var(--ui-font-mono);font-size:10px;text-align:center;text-decoration:underline;text-decoration-color:var(--ui-border-strong);text-underline-offset:3px;cursor:pointer}.transition-tab:hover{color:var(--ui-text);text-decoration-color:var(--ui-muted)}.transition-item.active .transition-tab{color:var(--ui-text);font-weight:700;text-decoration-color:var(--ui-accent)}.menu{width:100%;box-sizing:border-box;padding:4px}.menu-item{display:flex;width:100%;min-height:32px;box-sizing:border-box;padding:6px 8px;align-items:center;gap:8px;border:0;border-radius:4px;background:transparent;color:var(--ui-text);font:inherit;font-size:12px;text-align:left;cursor:pointer}.menu-item:hover:not(:disabled){background:var(--ui-surface-raised)}.menu-item:disabled{opacity:.4;cursor:not-allowed}.menu-item :global(svg){width:15px;height:15px}.menu-item.delete{color:var(--ui-danger)}.separator{height:1px;margin:4px;background:var(--ui-border)}
