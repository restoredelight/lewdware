<script lang="ts">
	...
	onMount(async () => {
		recents = await api.getRecentPacks();
	});
	...
	async function removeRecent(recent: RecentPack) {
		...
		try {
			...
			recents = recents.filter(
				(item) => (item.path ?? item.draft_id) !== (recent.path ?? recent.draft_id)
			);
		} catch (err) {
	...
	<section class="welcome" aria-labelledby="welcome-title">
		<header>
			...
			<div>
				...
				<p class="intro">
					Create and organise packs, their content, and the experience they provide.
				</p>
			</div>
		</header>
						...
						<div class="recent-row">
							...
							>
								...
								<span class="recent-copy">
									<strong
										>{recent.name}{#if !recent.path}<span class="draft-badge"
												>Recoverable draft</span
											>{/if}</strong
									>
									<small title={recent.path ?? 'Stored in local recovery data'}
										>{recent.path ??
											'Backed up locally · choose a destination on first save'}</small
									>
								</span>
							</button>
							<button
								class="recent-remove"
								type="button"
								aria-label={`Remove ${recent.name} from recent packs`}
								title="Remove from recent packs"
								onclick={() => removeRecent(recent)}><Icon src={XMark} mini size="14px" /></button
							>
						</div>
					{/each}
				</div>
