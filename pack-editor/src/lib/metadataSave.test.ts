...
const mocks = vi.hoisted(() => ({
  ...
  store: {
    ...
    markBackupComplete: vi.fn(), markBackupFailed: vi.fn(), markBackupPending: vi.fn(),
  },
...
const metadata = (name: string): MetadataDto => ({ name, creator: null, description: null, version: null, recommended_mode: null });
...
describe("metadata save scheduler", () => {
  ...
  it("retains failed metadata and succeeds when flushed again", async () => {
    mocks.api.savePackMetadata.mockRejectedValueOnce(new Error("disk full")).mockResolvedValue(undefined);
    const save = await scheduler();
    save.initializeMetadataHistory(metadata("Initial"));
    ...
    expect(mocks.feedback.error).toHaveBeenCalledWith("metadata-backup", expect.stringContaining("disk full"));
