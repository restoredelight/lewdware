<script lang="ts">
  ...
  const transition = $derived(store.behaviour!.experience!.timeline.transitions.find((item) => item.id === transitionId)!);
  ...
  $effect(() => { transitionId; rootEl?.scrollTo(0, 0); });
  const groups: { label: string; legacy: TransitionValue; values: { key: TransitionValue; label: string }[] }[] = [
    { label: "Event intervals", legacy: "events", values: [
      ...
      { key: "notification_interval", label: "Notifications" }, { key: "prompt_interval", label: "Prompts" },
      { key: "subliminal_interval", label: "Subliminals" },
    ] },
    { label: "Movement", legacy: "movement", values: [
      { key: "movement_minimum_speed", label: "Minimum speed" }, { key: "movement_maximum_speed", label: "Maximum speed" },
    ] },
    { label: "Mitosis", legacy: "mitosis", values: [
      { key: "mitosis_chance", label: "Chance" }, { key: "mitosis_count", label: "Number of copies" },
    ] },
  ];
  ...
  function affected(group: typeof groups[number], key: TransitionValue, checked: boolean) {
    ...
    if (transition.affected.includes(group.legacy)) {
      ...
      for (const value of group.values) if (!transition.affected.includes(value.key)) transition.affected.push(value.key);
    }
...
<div class="panel">
  ...
  <section class="card" class:disabled={transition.duration_seconds === 0}>
    <div class="section-title"><div><h3>Gradual changes</h3><p>Choose exactly which numeric values interpolate. All other changes take effect at the end.</p></div></div>
    {#each groups as group}
      <fieldset><legend>{group.label}</legend><div class="value-grid">
        {#each group.values as value}
          <label class="value"><Checkbox checked={isAffected(group,value.key)} disabled={transition.duration_seconds === 0} ariaLabel={`Gradually change ${value.label}`} onchange={(checked)=>affected(group,value.key,checked)}/><span>{value.label}</span></label>
        {/each}
      </div></fieldset>
    {/each}
  </section>
