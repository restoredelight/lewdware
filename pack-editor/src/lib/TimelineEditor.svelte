<script lang="ts">
  ...
  function addStage(){const source=stage;const next:Stage=source?clone(source):{id:"",label:"",content:{},events:{}};next.id=id("stage");next.label=`Stage ${stages.length+1}`;stages.splice(activeIndex+1,0,next);activeId=next.id;changed()}
...
<section class="layout">
  ...
  {#if stage}<main>
    ...
    <section class="card"><div class="section-title"><div><h3>Content selection</h3><p>Choose which tagged content and wallpaper are eligible during this stage.</p></div></div>
      <div class="toggle-row"><div><strong>Limit content by tag</strong>{#if previous}<small>Previous stage: {previous.content.tags?`${previous.content.tags.length} selected`:"All content"}</small>{/if}</div><Toggle ariaLabel="Limit content by tag" checked={!!stage.content.tags} onchange={(on)=>{if(on)stage.content.tags=[];else delete stage.content.tags;scheduleBehaviourSave()}}/></div>{#if stage.content.tags}<TagPicker tags={stage.content.tags} id={`stage-content-${stage.id}`}/>{/if}
      <div class="toggle-row"><div><strong>Override wallpaper selection</strong>{#if previous}<small>Previous stage: {previous.content.wallpaper_tags?`${previous.content.wallpaper_tags.length} selected`:"Pack default"}</small>{/if}</div><Toggle ariaLabel="Override wallpaper selection" checked={!!stage.content.wallpaper_tags} onchange={(on)=>{if(on)stage.content.wallpaper_tags=[];else delete stage.content.wallpaper_tags;scheduleBehaviourSave()}}/></div>{#if stage.content.wallpaper_tags}<TagPicker tags={stage.content.wallpaper_tags} id={`stage-wallpaper-${stage.id}`}/>{/if}
    </section>
    ...
    <section class="card"><div class="section-title"><div><h3>Stage duration</h3><p>{activeIndex===stages.length-1?"The final stage continues until the session ends.":"Choose how long these settings stay fully active."}</p></div></div>{#if stage.end}<div class="fields"><label>Keep these settings for (minutes)<input type="number" min="0" value={(stage.end.duration_seconds??300)/60} oninput={(e)=>{stage.end!.duration_seconds=e.currentTarget.valueAsNumber*60;scheduleBehaviourSave()}}/>{#if stage.end.duration_seconds===0}<small>The transition begins as soon as this stage is reached.</small>{/if}</label><label>Additional condition<Select label="Condition" value={stage.end.event_count?stage.end.event_count.event:"none"} options={[{value:"none",label:"No event condition"},...eventDefs.map(d=>({value:d.key,label:`${d.label} spawned`}))]} onchange={(v)=>{if(v==="none")delete stage.end!.event_count;else stage.end!.event_count={event:v as any,count:10,scope:"stage"};scheduleBehaviourSave()}}/></label>{#if stage.end.event_count}<label>Event count<input type="number" min="1" bind:value={stage.end.event_count.count} oninput={scheduleBehaviourSave}/></label><label>Advance when<Select label="Trigger strategy" value={stage.end.strategy} options={[{value:"any",label:"Either condition is reached"},{value:"all",label:"Both conditions are reached"}]} onchange={(v)=>{stage.end!.strategy=v as "any"|"all";scheduleBehaviourSave()}}/></label>{/if}</div>{#if next && outgoing}<div class="next-summary"><span>Then change to <button onclick={()=>activeId=next.id}>{next.label}</button></span><button class="transition-link" onclick={()=>activeId=outgoing.id}>{transitionSummary()} <span aria-hidden="true">→</span></button></div>{/if}{/if}</section>
  </main>{:else if transition && transitionFrom && transitionTo}<TransitionEditor {transition} from={transitionFrom} to={transitionTo} onstage={(id)=>activeId=id}/>{/if}
</section>
...
<style>
  .layout{display:flex;min-height:0;flex:1}.layout>aside{display:flex;width:192px;flex:none;padding:12px;flex-direction:column;gap:12px;border-right:1px solid var(--ui-border);background:var(--ui-surface)}.tabs{min-height:0;flex:1;overflow-y:auto}main{display:flex;width:100%;max-width:800px;min-width:0;padding:24px;overflow-y:auto;flex-direction:column;gap:14px}main>header{display:flex;align-items:start;justify-content:space-between;gap:16px}.stage-name{width:100%;padding:0;border:0;background:transparent;color:var(--ui-text);font-size:20px;font-weight:650}.stage-name:focus-visible{outline:2px solid var(--ui-focus);outline-offset:3px}header p,.section-title p{margin:4px 0 0;color:var(--ui-muted);font-size:12px}.card{display:flex;padding:16px;flex-direction:column;gap:12px;border:1px solid var(--ui-border);border-radius:var(--ui-radius-md);background:var(--ui-surface)}.section-title,.toggle-row{display:flex;align-items:center;justify-content:space-between;gap:16px}.section-title h3{margin:0;font-size:16px}.toggle-row{padding-top:10px;border-top:1px solid var(--ui-border)}.toggle-row strong,.toggle-row small{display:block;font-size:12px}.toggle-row small{margin-top:2px;color:var(--ui-muted);font-size:10px}.fields{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.fields label{display:flex;min-width:0;flex-direction:column;gap:5px;color:var(--ui-muted);font-size:11px}.fields input{width:100%;height:36px;padding:0 9px;border:1px solid var(--ui-border);border-radius:var(--ui-radius-sm);background:var(--ui-bg);color:var(--ui-text)}.fields small{font-size:10px}.next-summary{display:flex;padding-top:12px;align-items:center;justify-content:space-between;gap:12px;border-top:1px solid var(--ui-border);color:var(--ui-muted);font-size:12px}.next-summary button{padding:0;border:0;background:transparent;color:var(--ui-text);font:inherit;text-decoration:underline;text-decoration-color:var(--ui-border-strong);text-underline-offset:3px;cursor:pointer}.next-summary .transition-link{padding:7px 9px;border-radius:var(--ui-radius-sm);background:var(--ui-bg);text-decoration:none}.transition-link:hover{background:var(--ui-surface-raised)}@media(max-width:700px){.layout{flex-direction:column}.layout>aside{width:100%;padding-block:0;align-items:center;flex-direction:row;border-right:0;border-bottom:1px solid var(--ui-border)}.tabs{overflow-x:auto}main{padding:16px}.fields{grid-template-columns:1fr}.next-summary{align-items:flex-start;flex-direction:column}}
</style>
