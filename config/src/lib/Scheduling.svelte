<script lang="ts">
  ...
  function windowSummary(window: WindowDto): string {
    ...
    const timing = window.jitter_minutes > 0
      ? `between ${start} and ${windowEndValue(window)}`
      : `at ${start}`;
...
<div class="w-full max-w-4xl mx-auto flex flex-col gap-6 p-8">
  ...
  <section class="flex flex-col gap-3 border-t border-border pt-6">
    <div class="flex items-start justify-between gap-4">
      <div>
        ...
        <p class="m-0 mt-1 text-xs text-muted">Define when a session may start, how long it runs, and whether its start time varies.</p>
      </div>
      <Button size="compact" variant="secondary" onclick={() => store.addWindow()}>Add window</Button>
    </div>
    <div class="flex flex-col gap-3">
      {#each store.config?.schedule.windows ?? [] as window, i (i)}
        <Card class="flex flex-col gap-4 p-4">
          <div class="flex items-start justify-between gap-4">
            <div class="min-w-0">
              <h3 class="m-0 text-sm font-semibold text-text">Window {i + 1}</h3>
              <p class="m-0 mt-1 text-xs text-muted">{windowSummary(window)}</p>
            </div>
            <Button size="compact" variant="destructive" onclick={() => (pendingRemoval = { kind: "window", index: i })}>Remove</Button>
          </div>
          <div class="flex flex-col gap-1.5">
            ...
            {#if !window.days.some(Boolean)}<span class="text-xs text-[var(--ui-warning)]">Select at least one day for this window to take effect.</span>{/if}
          </div>
          <div class="flex flex-col gap-2">
            <span class="text-xs font-semibold text-text">Start timing</span>
            <div class="inline-flex self-start rounded-md border border-border bg-bg p-0.5" role="group" aria-label={`Start timing for window ${i + 1}`}>
              <button
                class="h-7 cursor-pointer rounded px-3 text-xs font-medium transition-colors {window.jitter_minutes === 0 ? 'bg-surface-2 text-text' : 'text-muted hover:text-text'}"
                aria-pressed={window.jitter_minutes === 0}
                onclick={() => setWindowStartMode(i, window, false)}
              >Start at</button>
              <button
                class="h-7 cursor-pointer rounded px-3 text-xs font-medium transition-colors {window.jitter_minutes > 0 ? 'bg-surface-2 text-text' : 'text-muted hover:text-text'}"
                aria-pressed={window.jitter_minutes > 0}
                onclick={() => setWindowStartMode(i, window, true)}
      ...
      {#if (store.config?.schedule.windows ?? []).length === 0}
        <Card class="flex flex-col items-center border-dashed !border-[var(--ui-border-strong)] p-7 text-center">
          <h3 class="m-0 text-sm font-semibold text-text">No session windows</h3>
          <p class="m-0 mt-1 mb-4 text-xs text-muted">Scheduling is enabled, but no sessions can start until you add a window.</p>
          <Button size="compact" variant="primary" onclick={() => store.addWindow()}>Add window</Button>
        </Card>
      {/if}
    </div>
  </section>
  ...
  <section class="flex flex-col gap-3 border-t border-border pt-6">
    <div class="flex items-start justify-between gap-4">
      <div><h2 class="ui-section-title">Quiet hours</h2><p class="m-0 mt-1 text-xs text-muted">Prevent scheduled activity during these times. Manually launched sessions are unaffected.</p></div>
      <Button size="compact" variant="secondary" onclick={() => store.addQuietHours()}>Add quiet hours</Button>
    </div>
    <div class="flex flex-col gap-3">
      {#each store.config?.schedule.quiet_hours ?? [] as quiet, i (i)}
        <Card class="flex flex-col gap-4 p-4">
          ...
          <div class="grid grid-cols-2 gap-3"><Field label="From" type="time" size="compact" value={toTimeValue(quiet.start_hour, quiet.start_minute)} onchange={(value) => { const t = fromTimeValue(value); if (t) store.updateQuietHours(i, { start_hour: t.hour, start_minute: t.minute }); }} /><Field label="Until" type="time" size="compact" value={toTimeValue(quiet.end_hour, quiet.end_minute)} onchange={(value) => { const t = fromTimeValue(value); if (t) store.updateQuietHours(i, { end_hour: t.hour, end_minute: t.minute }); }} /></div>
          {#if quiet.start_hour === quiet.end_hour && quiet.start_minute === quiet.end_minute}<span class="text-xs text-[var(--ui-warning)]">Start and end are the same, so this quiet period has no effect.</span>{/if}
        </Card>
      {/each}
      {#if (store.config?.schedule.quiet_hours ?? []).length === 0}
        <Card class="border-dashed !border-[var(--ui-border-strong)] p-5 text-center">
          <p class="m-0 text-xs text-muted">No quiet hours. Scheduled sessions may run during any configured window.</p>
        </Card>
      {/if}
    </div>
  </section>
  ...
  <section class="flex flex-col gap-3 border-t border-border pt-6">
    ...
    <Card class="flex items-center justify-between gap-4 p-4"><div><h3 class="m-0 text-sm font-medium text-text">Warn before starting</h3><p class="m-0 mt-1 text-xs text-muted">The notification includes an action to skip that occurrence.</p></div><Toggle checked={store.config?.schedule.grace_notification ?? false} ariaLabel="Warn before a scheduled session starts" onchange={(checked) => store.setGraceNotification(checked)} /></Card>
  </section>
  {:else}
    <Card class="flex flex-col items-center border-dashed !border-[var(--ui-border-strong)] p-8 text-center">
      <h2 class="m-0 text-sm font-semibold text-text">Scheduling is turned off</h2>
      <p class="m-0 mt-1 max-w-md text-xs leading-relaxed text-muted">Enable automatic scheduling above to review and edit session windows, quiet hours, and startup notifications.</p>
    </Card>
  {/if}
</div>
</div>
...
{#if pendingRemoval}
  <Dialog
    ...
    description={pendingRemoval.kind === "window" ? "Sessions will no longer be able to start during this window." : "Scheduled activity will no longer be blocked during this quiet period."}
    buttons={[{ label: "Cancel", onclick: () => (pendingRemoval = null) }, { label: "Remove", destructive: true, onclick: confirmRemoval }]}
