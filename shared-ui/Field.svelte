<script lang="ts">
  ...
  let { label, value = "", type = "text", description, error, placeholder, required = false, disabled = false, min, max, step, size = "normal", hideLabel = false, class: className = "", oninput, onchange }: Props = $props();
  const uid = $props.id();
  const id = `field-${uid}`;
  const describedBy = $derived([description ? `${id}-description` : "", error ? `${id}-error` : ""].filter(Boolean).join(" ") || undefined);
</script>

<label for={id} class={className}>
  <span class="label" class:sr-only={hideLabel}>{label}{#if required}<span class="required" aria-hidden="true"> *</span>{/if}</span>
  {#if description}<span id={`${id}-description`} class="description">{description}</span>{/if}
  <input
    {id} {type} {value} {placeholder} {required} {disabled} {min} {max} {step}
