...
<style>
  ...
  .panel { position: relative; width: min(400px, calc(100vw - 48px)); border: 1px solid var(--ui-border-strong); border-radius: var(--ui-radius-md); background: var(--ui-surface); box-shadow: var(--ui-shadow-pop); }
  /* the echo frame — one earlier "spawn" behind the live window */
  .panel::before { content: ""; position: absolute; inset: 0; z-index: -1; transform: translate(-10px, -10px); border: 1px solid var(--ui-border); border-radius: var(--ui-radius-md); background: rgb(10 8 9 / .4); }
  .panel:focus { outline: none; }
  .titlebar { display: flex; align-items: center; gap: 8px; height: 32px; padding: 0 10px; border-bottom: 1px solid var(--ui-border); background: var(--ui-surface-raised); border-radius: var(--ui-radius-md) var(--ui-radius-md) 0 0; }
  .dot { width: 8px; height: 8px; flex: none; border-radius: 50%; background: var(--ui-accent); }
  h2 { flex: 1; min-width: 0; margin: 0; overflow: hidden; color: var(--ui-text); font-family: var(--ui-font-mono); font-size: 11.5px; font-weight: 700; line-height: 1.3; text-overflow: ellipsis; white-space: nowrap; }
  .close { display: grid; width: 24px; height: 24px; flex: none; margin-right: -4px; padding: 0; place-items: center; border: 0; border-radius: var(--ui-radius-sm); background: transparent; color: var(--ui-muted); cursor: pointer; }
  ...
  .body { padding: 16px; }
  p { margin: 0 0 18px; color: var(--ui-muted); font-size: 13px; line-height: 1.45; }
