<script lang="ts">
	...
	type Props = {
		trigger: Snippet<[(event?: MouseEvent) => void, boolean]>;
	...
	function close(restore = true) {
		open = false;
		if (restore) tick().then(() => triggerElement?.focus());
	}
...
<style>
	...
	.panel :global([role='menuitem']) {
		cursor: pointer;
	}
	.panel :global([role='menuitem']:disabled) {
		cursor: not-allowed;
		opacity: 0.5;
	}
	.panel :global([role='menuitem']:hover),
	.panel :global([role='menuitem']:focus-visible) {
