...
pub fn try_import_drm_prime(
    ...
    let desc_ptr = unsafe { (*drm_prime.frame.as_ptr()).data[0] } as *const ffi::AVDRMFrameDescriptor;
    if desc_ptr.is_null() {
        return None;
    }
        ...
        && desc.layers[0].nb_planes == 2
    {
        let l = &desc.layers[0];
        (
            l.planes[0].offset as u64, l.planes[0].pitch as u64,
            l.planes[1].offset as u64, l.planes[1].pitch as u64,
        )
    ...
    {
        (
            desc.layers[0].planes[0].offset as u64, desc.layers[0].planes[0].pitch as u64,
            desc.layers[1].planes[0].offset as u64, desc.layers[1].planes[0].pitch as u64,
        )
    } else {
        eprintln!(
            "[drm_import] unhandled layout: nb_layers={} planes=[{}]",
            desc.nb_layers,
            (0..desc.nb_layers as usize).map(|i| desc.layers[i].nb_planes.to_string()).collect::<Vec<_>>().join(",")
        );
        return None;
    };
...
unsafe fn import_via_vulkan(
    ...
    let is_linear = modifier == 0 || modifier == u64::MAX;
    let has_modifier_ext = hal.enabled_device_extensions()
    ...
    if !is_linear && !has_modifier_ext {
        ...
        if !NOT_SUPPORTED.swap(true, Ordering::Relaxed) {
            eprintln!("[drm_import] non-linear modifier {modifier:#018x} but VK_EXT_image_drm_format_modifier not enabled — falling back to CPU");
        }
        return None;
    }
    ...
    let (y_image, uv_image) = if is_linear {
        let y = unsafe { create_linear_image(ash_device, vk::Format::R8_UNORM, video_width, video_height) }?;
        let y_row_pitch = unsafe {
            ash_device.get_image_subresource_layout(y, vk::ImageSubresource {
        ...
        let uv = match unsafe { create_linear_image(ash_device, vk::Format::R8G8_UNORM, chroma_w, chroma_h) } {
            Some(img) => img,
            None => { unsafe { ash_device.destroy_image(y, None) }; return None; }
        };
        let uv_row_pitch = unsafe {
            ash_device.get_image_subresource_layout(uv, vk::ImageSubresource {
                aspect_mask: vk::ImageAspectFlags::COLOR, mip_level: 0, array_layer: 0,
            })
        }.row_pitch as u64;
        if uv_row_pitch != uv_pitch {
            eprintln!("[drm_import] UV stride mismatch: Vulkan={uv_row_pitch} DRM={uv_pitch}");
            unsafe { ash_device.destroy_image(y, None); ash_device.destroy_image(uv, None) };
            return None;
        }
        (y, uv)
    } else {
        ...
        let y = match unsafe { create_modifier_image(ash_device, vk::Format::R8_UNORM, video_width, video_height, modifier, y_offset, y_pitch) } {
            Some(img) => img,
            None => return None,
        };
        let uv = match unsafe { create_modifier_image(ash_device, vk::Format::R8G8_UNORM, chroma_w, chroma_h, modifier, uv_offset, uv_pitch) } {
            Some(img) => img,
            None => { unsafe { ash_device.destroy_image(y, None) }; return None; }
        };
        (y, uv)
    ...
    if fd_dup < 0 {
        unsafe { ash_device.destroy_image(y_image, None); ash_device.destroy_image(uv_image, None) };
        return None;
    }
    let props_ok = unsafe {
        ext_mem_fd.get_memory_fd_properties(vk::ExternalMemoryHandleTypeFlags::DMA_BUF_EXT, fd_dup, &mut fd_props)
    };
    ...
    let compatible = y_mem_req.memory_type_bits & uv_mem_req.memory_type_bits & fd_props.memory_type_bits;
    ...
    let y_memory = {
        let mut import = vk::ImportMemoryFdInfoKHR::default()
            .handle_type(vk::ExternalMemoryHandleTypeFlags::DMA_BUF_EXT).fd(y_fd);
        ...
        match unsafe { ash_device.allocate_memory(&alloc, None) } {
            Ok(m) => m,
            Err(_) => {
                unsafe { libc::close(y_fd); ash_device.destroy_image(y_image, None); ash_device.destroy_image(uv_image, None) };
                return None;
            }
        }
    };
    if unsafe { ash_device.bind_image_memory(y_image, y_memory, y_bind_offset) }.is_err() {
        unsafe { ash_device.free_memory(y_memory, None); ash_device.destroy_image(y_image, None); ash_device.destroy_image(uv_image, None) };
        return None;
    }
    ...
    let uv_memory = {
        let mut import = vk::ImportMemoryFdInfoKHR::default()
            .handle_type(vk::ExternalMemoryHandleTypeFlags::DMA_BUF_EXT).fd(uv_fd);
        let alloc = vk::MemoryAllocateInfo::default()
            .allocation_size(total_size)
            .memory_type_index(mem_type as u32)
            .push_next(&mut import);
        match unsafe { ash_device.allocate_memory(&alloc, None) } {
            Ok(m) => m,
            Err(_) => {
                unsafe { libc::close(uv_fd); ash_device.free_memory(y_memory, None); ash_device.destroy_image(y_image, None); ash_device.destroy_image(uv_image, None) };
                return None;
            }
        }
    };
    if unsafe { ash_device.bind_image_memory(uv_image, uv_memory, uv_bind_offset) }.is_err() {
        unsafe { ash_device.free_memory(uv_memory, None); ash_device.free_memory(y_memory, None); ash_device.destroy_image(y_image, None); ash_device.destroy_image(uv_image, None) };
        return None;
    }
    ...
    let uv_tex_desc = wgpu::TextureDescriptor {
        ...
        size: wgpu::Extent3d { width: chroma_w, height: chroma_h, depth_or_array_layers: 1 },
        mip_level_count: 1, sample_count: 1,
        dimension: wgpu::TextureDimension::D2,
    ...
    let y_texture = unsafe {
        let hal_tex = hal.texture_from_raw(y_image, &hal_desc_y, None, wgpu::hal::vulkan::TextureMemory::Dedicated(y_memory));
        wgpu_device.create_texture_from_hal::<wgpu::hal::vulkan::Api>(hal_tex, &y_tex_desc)
    };
    let uv_texture = unsafe {
        let hal_tex = hal.texture_from_raw(uv_image, &hal_desc_uv, None, wgpu::hal::vulkan::TextureMemory::Dedicated(uv_memory));
    ...
    if !LOGGED_SUCCESS.swap(true, Ordering::Relaxed) {
        let path = if is_linear { "linear" } else { "tiled (modifier ext)" };
        eprintln!("[drm_import] DMA-BUF zero-copy active ({path}) — GPU→CPU→GPU round-trip eliminated");
    }
    Some(DrmImportedTextures { y_texture, uv_texture, bind_group, _drm_frame: drm_frame })
}
...
fn find_memory_type(mem_props: &vk::PhysicalDeviceMemoryProperties, type_bits: u32) -> Option<usize> {
