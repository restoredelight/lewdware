...
/// A bunch of fixes to make the config AppImage work
#[cfg(target_os = "linux")]
pub fn apply_wayland_preload_safeguards() {
    ...
    // Disable DMA-BUF and Compositing Mode rendering paths
    ...
    // Preload host's libwayland-client.so.0 if running under Wayland
...
/// Stops a command from opening a terminal window on Windows
#[cfg(windows)]
pub fn silence_command(cmd: &mut Command) {
    use std::os::windows::process::CommandExt;
    cmd.creation_flags(0x08000000); // CREATE_NO_WINDOW
}

#[cfg(not(windows))]
pub fn silence_command(_: &mut Command) {}
