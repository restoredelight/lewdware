...
# pack-editor's image encode pipeline. No prebuilt static Linux binary is published upstream, so
...
# pack-editor/THIRD_PARTY_LICENSES.md.
set -e
...
cmake --build "$TEMP_DIR/libavif/build" --config Release --parallel "$(nproc)"
